"""
adapter cache key randomly initialized
annotate line 277: self.adapter.weight = nn.Parameter(self.cache_key)
"""
...
class PROMPT_TA_RANDOMINIT(Base_SFDG):
    """PS + Adapter.
    """    
    ...
    def build_cache_model(self):
        with torch.no_grad():
            ...
            self.adapter = nn.Linear(self.cache_key.shape[1], self.cache_key.shape[0], bias=False).to(self.model.dtype).to(self.device)
