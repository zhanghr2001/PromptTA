...
class PROMPT_TA_NOSAMP(Base_SFDG):
    """PS + Adapter.
    """    
    ...
    def train_fc(self, data):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            new_labels = []
            for cls in range(self.n_cls):
                new_labels.append(torch.full((self.cfg.TRAINER.PROMPT_TA.K_PROPMT,), cls))

            new_labels = torch.cat(new_labels, dim=0)

            dataset = TensorDataset(prompt_text_features, new_labels)
