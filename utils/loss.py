...
class AngularPenaltySMLoss(nn.Module):
    ...
    def forward(self, fc, x, labels=None, return_logits=False):
        '''
        ...
        for W in fc.parameters():
            W = F.normalize(W, p=2, dim=1, eps=1e-8)
        ...
        if return_logits and labels == None:
            return nn.functional.softmax(wf, dim=1)
        ...
        if return_logits:
            return -torch.mean(L), nn.functional.softmax(wf, dim=1)
