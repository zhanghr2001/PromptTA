...
def load_clip_to_cpu(cfg):
    ...
    model_path = _download(url, cfg.MODEL.BACKBONE.PATH)
...
class TextEncoder(nn.Module):
    def __init__(self, cfg, clip_model, prompt_learner=None):
        ...
        CFG_TRAINER = cfg.TRAINER.NAME.upper()
        if "CLIP" in CFG_TRAINER:
            trainer = "CLIP"
        elif "PROMPT_STYLER" in CFG_TRAINER:
            trainer = "PROMPT_STYLER"
        elif "PROMPT_TA" in CFG_TRAINER:
            trainer = "PROMPT_TA"
        else:
